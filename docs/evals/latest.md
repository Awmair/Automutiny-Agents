# Latest release checks

> Scope: deterministic offline contract fixtures. These results prove schemas, thresholds, guards and architectural boundaries; they are not a live-model quality benchmark.

## Intake Brief

Cases: 60 · Result: PASS

| Metric | Result |
|---|---:|
| Practice-area accuracy | 100.0% |
| Fit-bucket accuracy | 100.0% |
| Disqualifier recall | 100.0% |
| Valid next-action rate | 100.0% |
| Safe reply rate | 100.0% |

## Document Routing

Cases: 60 · Result: PASS

| Metric | Result |
|---|---:|
| Document-type accuracy | 100.0% |
| Routing accuracy | 100.0% |
| Field extraction F1 | 100.0% |
| Evidence grounding | 100.0% |

## Stalled Work

Cases: 6 · Result: PASS

| Metric | Result |
|---|---:|
| Detection recall | 100.0% |
| False-positive rate | 0.0% |
| Brief numeric consistency | 100.0% |

